...
public class TopicServiceImpl implements TopicService {
    ...
    @Autowired
    public TopicServiceImpl(@Value("${cluster:local}") String cluster, final KafkaClusterRepository kafkaClusterRepository) {
    ...
    public Set<String> listOrphanedTopics(final Collection<Domain> domains) throws ExecutionException, InterruptedException {
        ...
        return clusterTopicNames.stream()
                .filter(clusterTopicName -> handleException(() -> isOrphanedTopic(clusterTopicName, topologyTopicNames)))
                .collect(Collectors.toSet());
    }
    ...
    public NewTopic createNewTopic(final Topic topic) {
        ...
        short replicationFactor = local ? 1 : topic.getReplicationFactor();

        return new NewTopic(topic.getFullName(), topic.getNumPartitions(), replicationFactor)
                .configs(config);
    }
    ...
    public boolean isOrphanedTopic(String clusterTopicName, Set<String> topologyTopicNames) throws ExecutionException, InterruptedException {
    ...
    public boolean hasNoActiveConsumerGroups(final String topicName) throws ExecutionException, InterruptedException {
