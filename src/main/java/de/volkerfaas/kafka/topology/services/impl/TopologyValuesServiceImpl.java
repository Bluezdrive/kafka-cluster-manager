...
public class TopologyValuesServiceImpl implements TopologyValuesService {
    ...
    public void addAdditionalValuesToTopic(final Triplet<Domain, Visibility, Topic> triplet) {
        ...
        addSchemaType(topic.getKeySchema());
        addSchemaType(topic.getValueSchema());
    }

    public void addSchemaType(final Schema schema) {
        if (Objects.nonNull(schema) && Objects.isNull(schema.getType())) {
            schema.setType(Schema.Type.findBySuffix(schema.getSuffix()));
        }
    }
