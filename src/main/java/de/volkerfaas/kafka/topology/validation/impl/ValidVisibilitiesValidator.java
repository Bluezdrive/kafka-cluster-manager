...
import de.volkerfaas.kafka.topology.utils.Visibilities;
...
public class ValidVisibilitiesValidator implements ConstraintValidator<ValidVisibilities, List<Visibility>> {
    ...
    @Override
    public boolean isValid(List<Visibility> visibilities, ConstraintValidatorContext context) {
        boolean result = true;
        context.disableDefaultConstraintViolation();

        for (Visibility.Type type : Visibility.Type.values()) {
            final long count = Visibilities.countVisibilitiesOfType(visibilities, type);
            if (count > 1) {
